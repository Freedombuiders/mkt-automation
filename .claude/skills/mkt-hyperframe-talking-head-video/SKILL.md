...
Pipeline đóng gói: từ MP4 quay sẵn (talking head 9:16) tới MP4 9:16 hoàn chỉnh đã render có captions, SFX, zoom hooks, b-roll toàn cảnh có hiệu ứng rich, và CTA finale. AUTOPILOT: scaffold → lint → render thẳng (KHÔNG preview) → báo absolute path của file MP4 cho user.
...
- User có footage talking-head muốn đóng gói thành MP4 9:16 hoàn chỉnh (autopilot, render thẳng, KHÔNG preview)
...
8. Lint ──────────────── npx hyperframes lint → fix any errors
    │
    ▼
9. Render MP4 ────────── npx hyperframes render -q standard -o <slug>.mp4 (KHÔNG mở preview)
    │
    ▼
10. Báo path ─────── in absolute path của MP4
```

**AUTOPILOT (override):** Theo `agents/videoeditor/CLAUDE.md` → mục AUTOPILOT. Skill chạy 1 mạch tới MP4, KHÔNG mở preview, KHÔNG dừng hỏi. Thiếu info thì tự chọn default (tỷ lệ mặc định 9:16). Render xong báo absolute path của file MP4 cho user.
...
Nếu auto-detect không tìm ra structure rõ ràng (vd vlog free-form), TỰ chia scene hợp lý theo gap/độ dài (KHÔNG dừng hỏi user) và chạy tiếp — autopilot.
...
## Step 8: Render MP4 (AUTOPILOT — KHÔNG preview)

KHÔNG chạy `npx hyperframes preview`. Render thẳng ra MP4:

```bash
cd workspace/content/YYYY-MM-DD/<slug>/
npx hyperframes render -q standard -o <slug>.mp4
# output: workspace/content/YYYY-MM-DD/<slug>/<slug>.mp4 (1080x1920 30fps)
```

Nếu render báo lỗi lint → fix lỗi (xem Step 7 + `references/pitfalls.md`) rồi render lại. Lặp tới khi ra MP4. Nếu render fail không khắc phục được → báo lỗi cho user.

Sau khi có MP4, báo absolute path của file MP4 cho user:

```
Render xong: <slug>.mp4 (1080x1920, ~<duration>s)
...
2. **AUTOPILOT render** — KHÔNG mở preview. Render thẳng `npx hyperframes render -q standard` ra MP4 rồi báo absolute path của file MP4 cho user. Xem mục AUTOPILOT trong `agents/videoeditor/CLAUDE.md`.
...
Sau Step 8 (render xong), dùng format này:

```markdown
## Video render xong ✅

**File:** `workspace/content/YYYY-MM-DD/<slug>/<slug>.mp4` (1080x1920 30fps, ~<duration>s)

**Đã apply:**
- N caption groups synced với word-level transcript
- 6 SFX (camera-flash @ 0s, snap/whoosh/laser cho 3 lesson, ting recap, discord CTA)
- Source video punch zooms + ambient ken-burns drifts
- Hook overlay 0-4s (face vẫn hiện)
- N full-screen lesson b-rolls với rich effects
- Recap card + CTA finale
