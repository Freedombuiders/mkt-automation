...
Tell the user three things in one short reply: absolute output path của file MP4, avatar look used, duration & file size.

**AUTOPILOT:** Nếu skill này chạy standalone (không phải là Phase 2 của 1 orchestrator khác như `mkt-full-video-*`), báo absolute path của file MP4 cho user.

Nếu là sub-step của pipeline khác → chỉ report path (orchestrator sẽ packaging tiếp).
