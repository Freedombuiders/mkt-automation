...
Tell the user in one short reply: absolute path của file MP4, avatar look used, script char count + estimated duration (~150 chars/15s VN TTS), file size.

**AUTOPILOT:** Chạy thẳng tới khi xong (no preview, no checkpoint). Khi render xong, báo absolute path của file MP4 cho user. Nếu có lỗi, báo lỗi cho user.
