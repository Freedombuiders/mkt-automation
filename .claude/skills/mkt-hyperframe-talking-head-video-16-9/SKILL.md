...
Dedicated **16:9 (1920×1080) landscape editor** — input là 1 folder workspace có sẵn `voiceover.mp3` + `source.mp4` + `script.txt`, output là MP4 1920×1080 đã render (`<slug>.mp4`) trong cùng folder. AUTOPILOT: scaffold → lint → render thẳng (KHÔNG preview) → báo path MP4.

Đây là layer **operational** dưới orchestrator `mkt-full-video-with-11-hyperframe-heygen-16-9`. Orchestrator handle Phase 1 (TTS) + Phase 2 (HeyGen avatar) → handoff folder cho skill này lo Phase 3 (transcribe + scaffold + lint + render MP4).
...
3. detect_scenes.py           ── transcript + script.txt → scenes-outline.json (5 scenes)
    │
    ▼
4. (AUTOPILOT — KHÔNG checkpoint) ── in 1 dòng tóm tắt outline rồi chạy thẳng
    │
    ▼
...
8. [optional] scaffold_infographic_slots.py
    ...
    │
    ▼
9. lint + render          ── npx hyperframes lint → 0 errors, then render -q standard
    │
    ▼
10. Báo path ─────── in absolute path của MP4
```

**AUTOPILOT (override):** Theo mục AUTOPILOT trong `agents/videoeditor/CLAUDE.md`. KHÔNG mở preview, KHÔNG dừng hỏi. Chạy 1 mạch tới MP4 1920×1080, render xong báo absolute path của file MP4 cho user. Thiếu input thì tự chọn default.
...
**AUTOPILOT — KHÔNG checkpoint.** In 1 dòng tóm tắt outline (5 scene + variants) rồi chạy thẳng. KHÔNG dừng hỏi user duyệt (như được gọi với `--auto`).
...
### Step 8: lint + render (AUTOPILOT — KHÔNG preview)
...
- Errors that must be fixed (fix rồi lint lại):
...
Khi lint 0 errors, render thẳng (KHÔNG `npx hyperframes preview`):

```bash
npx hyperframes render -q standard -o <slug>.mp4
# output: <workspace>/<slug>.mp4 (1920×1080 30fps)
```
...
/absolute/path/<workspace>/<slug>.mp4
```

Nếu render lỗi → báo lỗi cho user.
...
npx hyperframes render -q standard -o <slug>.mp4
# rồi báo absolute path của file MP4 cho user: /abs/<workspace>/<slug>.mp4
```
...
```
workspace/content/2026-05-08/loi-ich-claude-ai/
```
...
Sau Step 8 (render xong), print to user + báo absolute path của MP4:

```markdown
## Video render xong ✅ (16:9 landscape)

**File:** `<workspace>/<slug>.mp4` (1920×1080 30fps, ~<duration>s)
...
**Absolute path:** `/absolute/path/<workspace>/<slug>.mp4`
```
